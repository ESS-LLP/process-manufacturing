...
class ProcessOrder(Document):
    ...
    def get_process_details(self):
        #	Set costing_method
        self.costing_method = frappe.db.get_value(
            "Process Definition", self.process_name, "costing_method")
        #	Set Child Tables
        ...
        if process:
            ...
            if process.finished_products:
                self.add_item_in_table(
                    process.finished_products, "finished_products")
            if process.scrap:
    ...
    def set_se_items_finish(self, se):
        ...
        se_materials = frappe.get_doc(
            "Stock Entry", {"process_order": self.name, "docstatus": '1'})
        ...
        if se_materials:
            ...
            for item in self.materials:
                se = self.set_se_items(
                    se, item, se.from_warehouse, None, False)
        ...
        hourly_rate = frappe.db.get_value(
            "Workstation", self.workstation, "hour_rate")
        if hourly_rate:
        ...
        for item in self.finished_products:
            if item.quantity > 0:
                qty_of_total_production = float(
                    qty_of_total_production) + item.quantity
                if self.costing_method == "Relative Sales Value":
                    ...
                    if sale_value_of_pdt:
                        total_sale_value += float(sale_value_of_pdt) * \
                            item.quantity
                    else:
                        frappe.throw(
                            _("Selling price not set for item {0}").format(item.item))

        value_scrap = frappe.db.get_value(
            "Process Definition", self.process_name, "value_scrap")
        if value_scrap:
        ...
        for item in self.finished_products:
            se = self.set_se_items(se, item, None, se.to_warehouse, True,
                                   qty_of_total_production, total_sale_value, production_cost, True)
        ...
        for item in self.scrap:
            if value_scrap:
                ...
                se = self.set_se_items(
                    se, item, None, self.scrap_warehouse, False)

        return se

    def set_se_items(self, se, item, s_wh, t_wh, calc_basic_rate=False, qty_of_total_production=None, total_sale_value=None, production_cost=None, is_finished_item=False):
        if item.quantity > 0:
            ...
            se_item.is_finished_item = is_finished_item
            ...
            if calc_basic_rate:
                print("Calc_basic_rate")
                print(self.costing_method)
                if self.costing_method == "Physical measurement":
                    se_item.basic_rate = production_cost/qty_of_total_production
                    print("Item " + se_item.item_code)
                    print("Basic Rate " + str(se_item.basic_rate))
                elif self.costing_method == "Relative Sales Value":
                    print("Else Part")
                    sale_value_of_pdt = frappe.db.get_value(
                        "Item Price", {"item_code": item.item}, "price_list_rate")
                    se_item.basic_rate = (
                        float(sale_value_of_pdt) * float(production_cost)) / float(total_sale_value)
                else:
                    print("FInal Else")
        return se
...
def manage_se_submit(se, po):
    if po.docstatus == 0:
        frappe.throw(
            _("Submit the  Process Order {0} to make Stock Entries").format(po.name))
